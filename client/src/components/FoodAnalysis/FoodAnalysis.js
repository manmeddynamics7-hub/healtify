...
import { /* searchFood, recognizeFood */ } from '../../services/apiService';
import { analyzeFoodImage, analyzeFoodByName, generateRecipeFromIngredients } from '../../services/geminiService';
...
const FoodAnalysis = () => {
  ...
  const searchFoodApi = async (query) => {
    ...
    try {
      ...
      toast.loading('rainscare is analyzing food options...', { id: 'food-search' });
      ...
      if (error.message.includes('API key')) {
        toast.error('rainscare service configuration error. Please check setup.');
    ...
    if (file) {
      ...
      reader.onload = async (e) => {
        ...
        try {
          ...
          toast.loading('Analyzing food with rainscare...', { id: 'ai-analysis' });
  ...
  const handleAiAnalysis = async (foodName) => {
    ...
    try {
      ...
      toast.loading('Analyzing food with Rainscare...', { id: 'ai-text-analysis' });
  ...
  const addAiFoodToIntake = async () => {
    ...
    try {
      ...
      if (uploadedImage) {
        try {
          const compressionResult = await tempIntakeService.compressImage(uploadedImage);
      ...
      const foodData = tempIntakeService.formatFoodDataForStorage(aiAnalysisResult, compressedImage);

      // Add to temporary daily intake
      await tempIntakeService.addFoodEntry(foodData);
      ...
      toast.success(`${aiAnalysisResult.foodName} added to your daily intake! 🍽️`);
      ...
      toast.dismiss('add-food');
      toast.error('Error adding food to intake');
  ...
  const generateRecipes = async () => {
    ...
    try {
      ...
      toast.loading('Generating personalized recipes...', { id: 'recipe-generation' });
  ...
  const getSuitabilityForUser = (food) => {
    ...
    if (userProfile?.dietType === 'vegetarian' && food.type === 'non-vegetarian') {
      warnings.push('Not suitable for vegetarian diet');
    }

    // Check health conditions
    if (userProfile?.healthConditions?.includes('pcos') || userProfile?.healthConditions?.includes('pcod')) {
      if (food.sugar > 10) warnings.push('High sugar content - limit for PCOS/PCOD');
      if (food.carbs > 20) warnings.push('High carb content - monitor for PCOS/PCOD');
    }
      ...
      >
        ...
        >
          <div className="flex justify-between items-center mb-4 sm:mb-6">
            <h3 className="text-xl sm:text-2xl font-bold text-gray-800 line-clamp-2 mr-2">{food.name}</h3>
            <button
              onClick={onClose}
          ...
          <div className="flex justify-between items-center mb-4 sm:mb-6">
            ...
            <div className={`flex items-center space-x-1 px-3 py-1 rounded-full ${getFoodTypeColor(food.type)}`}>
              {getFoodTypeIcon(food.type)}
              <span className="text-sm font-medium capitalize">{food.type}</span>
            </div>
          </div>
          ...
          <div className="text-center mb-6 p-4 bg-gradient-to-r from-sage/10 to-light-green/10 rounded-2xl">
            <div className="flex items-center justify-center space-x-2 mb-2">
              ...
              <span className="text-3xl font-bold text-gray-800">{food.calories}</span>
  ...
  return (
    <div className="min-h-screen p-3 sm:p-4 md:p-6 max-w-6xl mx-auto relative">
      ...
      <ScrollAnimationWrapper animationType="fadeInUp" className="mb-6 sm:mb-8">
        ...
        >
          Analyze nutrition, get personalized recommendations, and discover healthy recipes
        </motion.p>
      ...
      >
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
            <motion.button
              ...
              whileHover={{ scale: isAnalyzing || !searchTerm.trim() ? 1 : 1.05 }}
        ...
        <div className="mt-3 sm:mt-4 flex flex-wrap gap-2">
          <span className="text-xs sm:text-sm text-gray-600 mr-2">Quick Analysis:</span>
          {['Banana', 'Chicken Breast', 'Rice', 'Avocado', 'Oatmeal'].map((food) => (
            <motion.button
              key={food}
              onClick={() => {
                setSearchTerm(food);
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              onKeyPress={(e) => {
                if (e.key === 'Enter') {
                  searchFoodApi(searchTerm);
                }
              }}
              placeholder="Search for food items (press Enter to search with Rainscare...)"
        ...
        {uploadedImage && (
          ...
          >
            <img
              src={uploadedImage}
              alt="Uploaded food"
              className={`w-32 h-32 object-cover rounded-2xl ${isAnalyzing ? 'opacity-50' : ''}`}
            />
      ...
      {aiAnalysisResult && (
        ...
        >
          ...
          <div className="bg-emerald-50 rounded-2xl p-6">
            <h3 className="text-xl font-semibold text-emerald-800 mb-4">{aiAnalysisResult.foodName}</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Nutrition Facts */}
              <div>
                <h4 className="font-semibold text-gray-700 mb-2">Nutrition Facts</h4>
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span>Calories:</span>
                    <span className="font-medium">{aiAnalysisResult.calories}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Protein:</span>
                    <span className="font-medium">{aiAnalysisResult.nutritionFacts?.protein || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Carbs:</span>
                    <span className="font-medium">{aiAnalysisResult.nutritionFacts?.carbs || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Fat:</span>
                    <span className="font-medium">{aiAnalysisResult.nutritionFacts?.fat || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Fiber:</span>
                    <span className="font-medium">{aiAnalysisResult.nutritionFacts?.fiber || 'N/A'}</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{aiAnalysisResult.healthScore}/10</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Serving Size:</span>
                    <span className="font-medium">{aiAnalysisResult.servingSize}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Healthy:</span>
                    <span className={`font-medium ${aiAnalysisResult.isHealthy ? 'text-emerald-600' : 'text-orange-600'}`}>
            ...
            {aiAnalysisResult.recommendation && (
              <div className="mt-4 p-4 bg-blue-50 rounded-xl">
                <h4 className="font-semibold text-blue-800 mb-2">Personalized Recommendation</h4>
                <p className="text-blue-700">{aiAnalysisResult.recommendation}</p>
              </div>
            )}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <motion.button
              ...
              whileHover={{ scale: isGeneratingRecipes || !recipeIngredients.trim() ? 1 : 1.05 }}
              whileTap={{ scale: isGeneratingRecipes || !recipeIngredients.trim() ? 1 : 0.95 }}
            >
              {isGeneratingRecipes ? (
                <>
                  <div className="animate-spin rounded-full h-5 w-5 border-2 border-white border-t-transparent mr-2" />
            ...
            {suggestedRecipes.map((recipe, index) => (
              ...
              >
                <div className="flex items-start justify-between mb-2 sm:mb-3">
                  <h3 className="font-semibold text-gray-800 flex-1 text-sm sm:text-base line-clamp-2 mr-2">{recipe.recipeName}</h3>
                  <div className="flex items-center space-x-1 text-sage flex-shrink-0">
                    <Star className="w-3 h-3 sm:w-4 sm:h-4 fill-current" />
                    <span className="text-xs sm:text-sm">{recipe.healthScore}/10</span>
                  </div>
                </div>
                ...
                <div className="space-y-1 sm:space-y-2 mb-3 sm:mb-4">
                  <div className="flex items-center space-x-2 text-xs sm:text-sm text-gray-600">
                    ...
                    <span>{recipe.cookingTime} • {recipe.difficulty}</span>
                  </div>
                  <div className="flex items-center space-x-2 text-xs sm:text-sm text-gray-600">
                ...
                >
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="font-semibold text-gray-800 text-sm sm:text-base leading-tight line-clamp-2 flex-1 mr-2">{food.name}</h3>
                      <div className={`flex items-center space-x-1 px-2 py-1 rounded-full ${getFoodTypeColor(food.type)} flex-shrink-0`}>
                        {getFoodTypeIcon(food.type)}
                      </div>
                    </div>

                    <p className="text-xs sm:text-sm text-gray-600 mb-2 sm:mb-3">{food.servingSize}</p>

                    {/* Calories */}
                    <div className="flex items-center justify-between mb-2 sm:mb-3">
                      <div className="flex items-center space-x-1">
                        <Flame className="w-3 h-3 sm:w-4 sm:h-4 text-orange-500" />
                        <span className="font-bold text-gray-800 text-sm sm:text-base">{food.calories} cal</span>
                      </div>
                    ...
                    <div className="flex justify-between items-center mt-auto">
                      {suitability.warnings.length > 0 && (
                        <div className="flex items-center space-x-1 text-red-600">
                          ...
                          <span className="text-xs">{suitability.warnings.length} warning{suitability.warnings.length > 1 ? 's' : ''}</span>
                        </div>
                      )}
                      ...
                      {suitability.benefits.length > 0 && (
                        <div className="flex items-center space-x-1 text-green-600">
                          ...
                          <span className="text-xs">{suitability.benefits.length} benefit{suitability.benefits.length > 1 ? 's' : ''}</span>
                        </div>
                      )}
                    </div>
                  </div>
      ...
      {!searchTerm && searchResults.length === 0 && !isLoading && (
        ...
        >
          <div className="flex justify-center space-x-4 mb-6">
            ...
            <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl flex items-center justify-center floating-animation" style={{ animationDelay: '0.5s' }}>
          ...
          <p className="text-gray-600 mb-6">
            Search for food items or upload a photo to get detailed nutritional information
          </p>
          <div className="space-y-2 text-sm text-gray-500">
            <p>✨ Get personalized recommendations based on your health profile</p>
            ...
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mb-4 sm:mb-6">
                <div className="bg-orange-50 rounded-2xl p-3 sm:p-4 text-center">
                  ...
                  <p className="text-base sm:text-lg font-bold text-orange-600">{aiAnalysisResult.calories}</p>
                </div>
                <div className="bg-blue-50 rounded-2xl p-3 sm:p-4 text-center">
                  ...
                  <p className="text-base sm:text-lg font-bold text-blue-600">{aiAnalysisResult.nutritionFacts.protein}</p>
                </div>
                <div className="bg-green-50 rounded-2xl p-3 sm:p-4 text-center">
                  ...
                  <p className="text-base sm:text-lg font-bold text-green-600">{aiAnalysisResult.nutritionFacts.carbs}</p>
                </div>
                <div className="bg-yellow-50 rounded-2xl p-3 sm:p-4 text-center">
                  ...
                  <p className="text-base sm:text-lg font-bold text-yellow-600">{aiAnalysisResult.nutritionFacts.fat}</p>
                </div>
                <div className="bg-purple-50 rounded-2xl p-3 sm:p-4 text-center">
                  ...
                  <p className="text-base sm:text-lg font-bold text-purple-600">{aiAnalysisResult.nutritionFacts.fiber}</p>
                </div>
                <div className="bg-red-50 rounded-2xl p-3 sm:p-4 text-center">
                  ...
                  <p className="text-base sm:text-lg font-bold text-red-600">{aiAnalysisResult.nutritionFacts.sodium}</p>
                </div>
              </div>

              {/* Recommendation */}
              <div className="bg-gray-50 rounded-2xl p-4 mb-4">
                <h4 className="font-semibold text-gray-800 mb-2">Rainscare Recommendation</h4>
                <p className="text-gray-700">{aiAnalysisResult.recommendation}</p>
              </div>

              {/* Health Warnings */}
              {aiAnalysisResult.healthWarnings && aiAnalysisResult.healthWarnings.length > 0 && (
              ...
              {/* Health Benefits */}
              {aiAnalysisResult.healthBenefits && aiAnalysisResult.healthBenefits.length > 0 && (
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-6">
                {aiAnalysisResult.suitableFor && aiAnalysisResult.suitableFor.length > 0 && (
                  <div className="bg-blue-50 rounded-2xl p-4">
                    <h4 className="font-semibold text-blue-800 mb-2">Good For:</h4>
                    <ul className="text-blue-700 space-y-1">
                      {aiAnalysisResult.suitableFor.map((condition, index) => (
                ...
                {aiAnalysisResult.avoidIf && aiAnalysisResult.avoidIf.length > 0 && (
                  <div className="bg-orange-50 rounded-2xl p-4">
                    <h4 className="font-semibold text-orange-800 mb-2">Avoid If:</h4>
              ...
              {aiAnalysisResult.alternatives && aiAnalysisResult.alternatives.length > 0 && (
                <div className="bg-yellow-50 rounded-2xl p-4 mb-6">
                  <h4 className="font-semibold text-yellow-800 mb-2">Healthier Alternatives:</h4>
                  <ul className="text-yellow-700 space-y-1">
                    {aiAnalysisResult.alternatives.map((alternative, index) => (
          ...
          >
            ...
            >
              ...
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-gray-800">{selectedRecipe.recipeName}</h2>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {selectedRecipe.tips.map((tip, index) => (
                        <div key={index} className="flex items-start space-x-2 p-3 bg-blue-50 rounded-xl">
