...
const ProgressTracker = () => {
  ...
  const { dailyData, getWeeklyData, getMonthlyData, updateDailyMetric } = useHealthData();
  ...
  const WeeklyChart = ({ data, metric, color }) => {
    const maxValue = Math.max(...data.map(day => day[metric] || 0), 1);
  ...
  const NutritionChart = ({ data }) => {
    const totalCalories = data.calories || 1;
    const proteinCals = (data.protein || 0) * 4;
    const carbsCals = (data.carbs || 0) * 4;
    const fatCals = (data.fat || 0) * 9;
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-medium">{data.protein || 0}g ({proteinPercent.toFixed(0)}%)</span>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-medium">{data.carbs || 0}g ({carbsPercent.toFixed(0)}%)</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <motion.div
              initial={{ width: 0 }}
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-medium">{data.fat || 0}g ({fatPercent.toFixed(0)}%)</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <motion.div
              initial={{ width: 0 }}
    ...
    <div className="min-h-screen p-4 md:p-6 max-w-7xl mx-auto relative">
              ...
              <div className="glass-card p-6">
                ...
                {dailyData.foodEntries.length > 0 ? (
                  <div className="space-y-3 max-h-64 overflow-y-auto">
                    {dailyData.foodEntries.map((entry, index) => (
                      <div
                        key={entry.id}
                      ...
                      >
                        ...
                        <div className="text-right">
                          <p className="font-semibold text-green-600">{entry.calories} cal</p>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        Avg: {calculateWeeklyAverage(weeklyData, goal.key)} {goal.unit}
                      </p>
                    </div>
                  </div>
      ...
      {selectedPeriod === 'month' && (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {dailyGoals.map((goal, index) => {
              const monthlyAvg = calculateWeeklyAverage(monthlyData, goal.key);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { 
              ...
              icon: '🎯',
              achieved: dailyData.calories > 0 || dailyData.water > 0,
            ...
            { 
              ...
              icon: '👟',
              achieved: dailyData.steps >= 10000,
            ...
            { 
              ...
              icon: '🥗',
              achieved: dailyData.protein > 0 && dailyData.carbs > 0 && dailyData.fat > 0,
            ...
            { 
              ...
              icon: '🏆',
              achieved: weeklyData.filter(day => day.calories > 0).length >= 7,
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-gray-600">
                <li className="flex items-center space-x-2">
                  ...
                  <span>You've logged {dailyData.foodEntries.length} meals today</span>
                </li>
                <li className="flex items-center space-x-2">
                  <CheckCircle className="w-4 h-4 text-green-500" />
                  <span>Average weekly calories: {calculateWeeklyAverage(weeklyData, 'calories')}</span>
                </li>
                <li className="flex items-center space-x-2">
                  <CheckCircle className="w-4 h-4 text-green-500" />
                  <span>Water intake is {dailyData.water >= 2.0 ? 'excellent' : 'needs improvement'}</span>
                </li>
              </ul>
            </div>
