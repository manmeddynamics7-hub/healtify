  ...
  const addAiRecipeToIntake = async () => {
    ...
    try {
      const foodEntry = {
        ...
        serving_size: `${aiRecipe.servings} servings`,
        analysis_type: 'ai_recipe',
        health_score: aiRecipe.healthScore,
        recipe_data: aiRecipe
      };

      await addFoodEntry(foodEntry);
      toast.success(`${aiRecipe.recipeName} added to your daily intake!`);
      setShowAiRecipeModal(false);
    } catch (error) {
      toast.error('Error adding recipe to intake');
