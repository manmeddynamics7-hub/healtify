...
const Updates = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingUpdate 
        ? `http://localhost:5000/api/admin/updates/${editingUpdate.id}`
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this update?')) {
      try {
        const response = await fetch(`http://localhost:5000/api/admin/updates/${id}`, {
          method: 'DELETE',
          headers: {
            'x-admin-api-key': 'rainscare_admin_key_2024'
          }
  ...
  const toggleStatus = async (update) => {
    try {
      const response = await fetch(`http://localhost:5000/api/admin/updates/${update.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'x-admin-api-key': 'rainscare_admin_key_2024'
